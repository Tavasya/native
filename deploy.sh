...
gcloud run deploy livekit-agent \
    ...
    --set-env-vars LIVEKIT_URL=$LIVEKIT_URL,LIVEKIT_API_KEY=$LIVEKIT_API_KEY,LIVEKIT_API_SECRET=$LIVEKIT_API_SECRET,OPENAI_API_KEY=$OPENAI_API_KEY,DEEPGRAM_API_KEY=$DEEPGRAM_API_KEY,ELEVEN_API_KEY=$ELEVENLABS_API_KEY \
    --memory 2Gi \
    --cpu 2 \
    --min-instances 1 \
    --max-instances 3 \
